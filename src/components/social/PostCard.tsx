...
const handleCopyPostLink = async () => {
  ...
  return (
    <>
      <div onClick={() => navigate(`/p/${post.id}`)} role="button" className="px-4 py-4 border-b border-slate-200 dark:border-slate-800 bg-transparent hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <Link to={profileHref} className="font-semibold truncate hover:underline inline-flex items-center gap-2 flex-wrap">
                  <span>{author?.display_name || authorUsername}</span>
                  {author?.verified && <VerifiedBadge size={14} />}
                </Link>
                <div className="mt-0.5 flex items-center gap-2 flex-wrap text-sm text-slate-500 dark:text-slate-400 flex-wrap">
                    ...
                    <div className="absolute right-0 mt-2 w-44 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-lg overflow-hidden z-10">
                      {canDelete ? (
                        <button
                          ...
                          className="w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center gap-2 flex-wrap">
                ...
                {(comments || []).map((c: any) => {
                  ...
                  return (
                    <div key={c.id} className="flex gap-3">
                      ...
                      />
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap flex-wrap">
