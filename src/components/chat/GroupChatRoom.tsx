 ...
 * Uses non-global regex for test to avoid stateful .test() bugs.
 */
const renderTextWithLinks = (text: string) => {
  const splitRegex = /(https?:\/\/[^\s]+)/g
  const testRegex = /^https?:\/\/[^\s]+$/ // no "g" ✅
...
export default function GroupChatRoom({
  ...
  // ✅ Prefill textarea (safe) when initialText exists but not auto sending
  ...
  }, [initialText, autoSendInitial])

  // ✅ Auto-send shared link once (fixes blank share-to-group experience)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        await send({ content: t, message_type: 'text' } as any)
      } catch {
        // ignore
      }
    })()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoSendInitial, initialText, loading])
  ...
  const addMemberByUsername = async () => {
    ...
    const { error } = await supabase
      .from('group_members')
      .insert({ group_id: groupId, user_id: p.id, role: 'member' })
            ...
            <div className="max-h-[60vh] overflow-y-auto p-2">
              {members.map((m) => {
                ...
                return (
                  <div
                    key={m.user_id}
                    className="flex items-center justify-between gap-2 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-900"
                  >
                    <button
            ...
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className="max-w-[86%]">
                ...
                >
                  ...
                  {m.message_type === 'text' ? (
                    <div className="whitespace-pre-wrap break-words">{renderTextWithLinks(m.content)}</div>
                  ) : null}
        ...
        <div className="shrink-0 px-3 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/30">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0 text-xs text-slate-700 dark:text-slate-200">
              Replying to:{' '}
              <span className="font-semibold">{replyTo.message_type === 'text' ? replyTo.content : 'Voice note'}</span>
            </div>
